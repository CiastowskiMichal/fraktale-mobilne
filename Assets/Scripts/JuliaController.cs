...
public class JuliaController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        materialTarget.SetFloat("_Zoom", 80);
        materialTarget.SetVector("_Pan", new Vector3(0, 0, 0));
        materialTarget.SetFloat("_SeedX", 0);
        materialTarget.SetFloat("_SeedY", 0);
        seedSlider.value = 0;
    }
        ...
        if (Input.touchCount > 0)
        {
            ...
            if (inputArea.Contains(touchPos))
            {
                ...
                if (Input.touchCount == 1)
                {
                    ...
                    else if(seedSlider.value>0.5f)
                    {
                        //Touch touchZero = Input.GetTouch(0);
                        float x = (touchPos.x / (0.9f * (float)Screen.width)) * 2f - 1f;
                        float y = (touchPos.y / ((float)Screen.height)) * 2f - 1f;
                        Debug.Log(x + "   " + y);
                        materialTarget.SetFloat("_SeedX", x);
                        materialTarget.SetFloat("_SeedY", y);
                        // if (touchZero.phase == TouchPhase.Moved)
                        // {
                            //Vector2 touchDelta = touchZero.deltaPosition;
